...
const getSongs = async () => {
    ...
    for( i = 0; i < as.length; i++){
        ...
        if(element.href.endsWith(".mp3")){
            ...
            const clearUrl = element.href.slice(28) 
...
const main = async () => {
    ...
    //    let cards =  Array.from(document.querySelector(".pl-card").getElementsByClassName("card-style"))
    //     cards.forEach(e => {
    //     let cardTitle = e.querySelector("h3").innerText;
    //     let cardPara = e.querySelector("p").innerText;
    //     let cardImg = e.querySelector("img").src;

    //     let titleArr = []
    //     titleArr.push(cardTitle)
    //     let paraArr = []
    //     paraArr.push(cardPara)
    //     let imgArr = []
    //     imgArr.push(cardImg)
    ...
    //             if((currentSong.src.slice(28).replaceAll("%20", " ")).includes(titleArr)){
    //                 document.querySelector(".songInfo").innerHTML = `<img src="${cardImg}" width="50vw"> ${cardTitle} / ${cardPara}`
    //             }
    //         }
    //    })
    //    console.log(cards);
